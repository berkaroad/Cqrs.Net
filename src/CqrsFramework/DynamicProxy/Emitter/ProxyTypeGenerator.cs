...
namespace CqrsFramework.DynamicProxy.Emitters
{
    public class ProxyTypeGenerator : IProxyTypeGeneratorInfo
    {
        ...
        public ProxyTypeGenerator(ModuleBuilder moduleBuilder, Type proxiedInterfaceType, Type proxiedType)
        {
            _proxiedInterfaceType = proxiedInterfaceType;
            ProxiedType = proxiedType;
            Builder = moduleBuilder.DefineType(string.Format("{0}__DynamicProxy", proxiedType.Name), TypeAttributes.Public | TypeAttributes.Sealed, null, new Type[]
            {
