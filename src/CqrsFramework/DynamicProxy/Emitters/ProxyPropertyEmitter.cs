...
{
    public class ProxyPropertyEmitter : AbstractProxyMemberEmitter
    {
        ...
        public override void Emit(MemberInfo memberInfo)
        {
            ...
            var interceptorTypesField = _typeGeneratorInfo.GetField(Consts.INTERCEPTOR_TYPES_FIELD_NAME);
            ...
            if (proxiedTypePropertyInfo.CanRead)
            {
                ...
                generator.Emit(OpCodes.Call, typeof(AbstractProxyMemberEmitter).GetMethod("CreateInterceptors", new Type[] { typeof(Type[]) }));
                ...
                generator.Emit(OpCodes.Callvirt, typeof(IInvocation).GetMethod("Proceed"));

                var returnObj = generator.DeclareLocal(proxiedTypePropertyInfo.PropertyType);
                generator.Emit(OpCodes.Ldloc, invocationVar);
                generator.Emit(OpCodes.Callvirt, typeof(IInvocation).GetProperty("ReturnValue").GetMethod);
                generator.Emit(OpCodes.Callvirt, typeof(WrapperObject).GetProperty("Value").GetMethod);
